...
class TableBuilder:
    ...
    def _make_table(cls, ctx: BuildContext, table_def: dict[str, Any]) -> Table:
        ...
        for f in table_def.get("Fields") or []:
            col = build_column(ctx, name, f, pk_names)
            if col is not None:
...
def build_column(
    ...
    Side effect: appends deferred ``_PendingRef`` entries to ``ctx.pending_refs``
    for any ``TableRelation`` clauses on the field. Returns ``None`` for fields
    without a usable ``Name``.

    Exposed as a module-level function (not a method on ``TableBuilder``)
    because :class:`al2dbml._build.extensions.ExtensionBuilder` also needs to
    build columns from extension field definitions; column construction is the
    truly stateless inner loop shared between table and extension paths.
    """
